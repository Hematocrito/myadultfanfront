...
export default function MediaAll() {
  ...
  const getVideos = async()=>{  
    ...
    const response = await fetch(`http://api-temp.myadultfan.com/platform/videos/free/${filterName}/${gender}/${sort}`)
  ...
  return (
    <>
      <Navbar />
      <LayoutClipsHome filter={filter} setFilter={setFilter} getVideos={getVideos}>
        <div className="grid md:grid-cols-3 grid-cols-1 mb-6 md:mb-8">
          {videos?.map((model: any) => (
            <div key={model.id} className='md:mx-10 mx-3 my-10 md:w-96 w-full h-56'>
              ...
              <div className='w-96 h-44 border'>
